...
import { edit } from '../api/albums.js';
...
async function onSubmit(ctx, formData, event) {
    ...
    try {
        let updatedAlbum = await edit(ctx.album._id, { name, imgUrl, price, releaseDate, artist, genre, description });
        event.target.reset();
        ctx.page.redirect(`/catalog/${updatedAlbum._id}`);
