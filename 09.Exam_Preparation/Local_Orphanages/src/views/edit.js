...
async function onSubmit(ctx, formData, event) {
    ...
    try {
        let updatedPost = await edit(ctx.post._id, { title, description, imageUrl, address, phone });
        event.target.reset();
        ctx.page.redirect(`/catalog/${updatedPost._id}`);
